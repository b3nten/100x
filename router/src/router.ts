import {type HrefBuilderArgs, RegExpMatcher, RoutePattern} from "@remix-run/route-pattern";
import type {RoutesWithHandlerType, BuildRouteMap, RouteDefs, RouteHandler, RouteHandlersMap, RouteMap} from "./types.ts";
...
export class Routes<P extends string, const R extends RouteDefs> {
    ...
    >(): Handlers extends RouteHandlersMap<typeof this["definitions"]> ?
        RoutesWithHandlerType<typeof this["definitions"], Handlers> : never
    {
...
export class Router<T extends BuildRouteMap | RoutesWithHandlerType<any, any>, H extends RouteHandlersMap<T>> {
    static createURL = (
        path: string | URL
    ): URL => path instanceof URL
        ? path
        : URL.canParse(path)
            ? new URL(path)
            : new URL(path, typeof location !== "undefined" ? location.origin : "http://0.0.0.0")
    ...
    private map<T extends RouteMap>(routes: T, handlers: RouteHandlersMap<T>): void
