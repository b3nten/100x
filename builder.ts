...
export class Builder {
    ...
    async build(...options: Partial<esbuild.BuildOptions>[]) {
        ...
        return await Promise.all(options.map(o => esbuild.build({ ...this.config, ...o })))
    ...
    async buildTypes() {
        return await new Promise((resolve) => {
            const process = spawn('tsc', ['--declaration', '--emitDeclarationOnly'], {stdio: 'inherit'})
